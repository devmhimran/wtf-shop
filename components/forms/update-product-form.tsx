...
export function UpdateProductForm({ data }: UpdateProductFormProps) {
  ...
  >([]);
  const [variantMode, setVariantMode] = useState<'bulk' | 'individual'>(
    'individual'
  );
  ...
  useEffect(() => {
    if (data && !isDataLoaded) {
      ...
      if (product.mainImage) {
        setFeaturedImage(product.mainImage as MediaType);
        form.setValue('featuredImage', {
          id: product.mainImage.id,
          fileUrl: product.mainImage.fileUrl,
          fileName: product.mainImage.fileName,
        });
      }

      // Set alternative image
      if (product.alternativeImage) {
        setAlternativeImage(product.alternativeImage as MediaType);
        form.setValue('alternativeImage', {
          id: product.alternativeImage.id,
          fileUrl: product.alternativeImage.fileUrl,
          fileName: product.alternativeImage.fileName,
        });
      }
      ...
      if (product.metaKeyword) {
        const keywords = product.metaKeyword
          .split(',')
          .map((k: string) => k.trim());
        setMetaKeywords(keywords.map((k) => ({ value: k, label: k })));
        form.setValue('metaKeyword', keywords);
      }
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const response = updateProductAsync({ id: productId, updateData: values });
    ...
    toast.promise(response, {
      ...
      success: (response) => {
        ...
        return response.message || 'Successfully updated Product!';
      },
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
        <div className='flex items-center justify-between'>
          <h1 className='text-xl md:text-3xl font-bold'>Update Product</h1>
                      ...
                      >
                        {alternativeImage ? (
                          ...
                          <div className='flex flex-col items-center justify-center text-center'>
                            <FileImage className='w-12 h-12 text-muted-foreground mb-2' />
                            <p className='text-sm font-medium'>
                              Click to select alternative image
                            </p>
                            <p className='text-xs text-muted-foreground mt-1'>
                              Choose from media library or upload new
                            </p>
