...
export function UpdatePromoCodeForm({
  ...
  // Parse existing dates from data using dayjs
  ...
  const [endDate, setEndDate] = useState<Date | undefined>(existingEndDate);

  const extractTime = (dateString: string | undefined, defaultTime: string) => {
    if (!dateString) return defaultTime;
    return dayjs(dateString).format('HH:mm:ss');
  };

  const [startTime, setStartTime] = useState(
    extractTime(data?.startDate, '00:00:00')
  );
  const [endTime, setEndTime] = useState(
    extractTime(data?.endDate, '23:59:59')
  );
  ...
  const { updatePromoCodeAsync } = usePromoCode();

  const combineDateAndTime = (date: Date, time: string) => {
    const dateStr = dayjs(date).format('YYYY-MM-DD');
    return dayjs(`${dateStr} ${time}`).toISOString();
  };

  function onSubmit(formData: z.infer<typeof promoCodeSchema>) {
    if (!data?.id) return;
    const response = updatePromoCodeAsync({
      id: data?.id,
      updateData: formData,
    });
