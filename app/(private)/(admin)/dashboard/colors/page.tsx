...
import { CreateColorsForm } from '@/components/forms';
import { ColorsCard } from '@/components/pages/colors';
import { AlertModal } from '@/components/shared';
import { ColorsSkeleton } from '@/components/skeletons';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { useGetAllColors } from '@/hooks';
...
export default function ColorsPage() {
  ...
  const { fetchAllColorsMutation, fetchAllColorsMutationData } =
  ...
  return (
    <div className='space-y-6 w-full md:w-5/6 lg:w-3/6 mx-auto '>
      <div className='flex items-center justify-between'>
        <h1 className='text-xl md:text-3xl font-bold'>Colors</h1>
      ...
      <Card>
        ...
        <CardContent>
          {fetchAllColorsMutation.isLoading ? (
            <ColorsSkeleton />
          ) : (
            <ColorsCard data={fetchAllColorsMutationData?.data || []} />
          )}

          {fetchAllColorsMutationData &&
            fetchAllColorsMutationData.meta.count > 0 && (
                ...
                <div className='flex items-center space-x-2'>
                  ...
                  <Button
                    ...
                    disabled={
                      +params.page ===
                      (fetchAllColorsMutationData &&
                        fetchAllColorsMutationData.meta.totalPages)
                    }
                  >
                    Next
      ...
      <AlertModal
        ...
        description=' '
      >
        <CreateColorsForm setIsOpen={setAddColorModalOpen} />
      </AlertModal>
    </div>
  );
}
